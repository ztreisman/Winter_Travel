...
ggplot(by_zone, aes(past3snow, user.count, color = Zone))+
  ...
  facet_grid(modality~year)+
  scale_y_log10()+
  labs(x="Past 3 Days Snow (cm)", y="# Users")
```

#  Air Temperature

Effects of weather and conditions are subtle but not absent.
...
ggplot(by_zone, aes(air_temp, user.count, color = Zone))+
  ...
  facet_grid(modality~year)+
  scale_y_log10()+
  labs(x="Air Temperature (C)", y="# Users")
```
...
```{r}
model1 <- glm.nb(user.count ~ modality*(Zone+rating_near)+air_temp+change_depth+snow_depth+snow_density+weekend, data=by_zone)
anova(model1)
#summary(model1)
...
Usage has remained fairly stable.
